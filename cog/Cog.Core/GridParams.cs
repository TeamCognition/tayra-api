    ...
    public class GridParams
    {
        ...
        public bool IsValidPropertyName<T>() where T : class
        {
            ...
            return props.Any(p => Sidx.ToLower() == p) &&
                ...
                && Filters.Where(x => x.Type == Filter.FilterTypes.Match).All(x => x.Values.All(v => IsNumeric(v) || IsBoolean(v))) //maybe determine prop type by name and check with that?
                && Filters.Where(x => x.Type == Filter.FilterTypes.Range).All(x => x.Values.All(v => IsNumeric(v) || IsDate(v)))
                && Filters.Where(x => x.Type == Filter.FilterTypes.Like).All(x => x.Values.All(v => v.ToString().All(char.IsLetterOrDigit)))));
