...
namespace Cog.DAL
{
    ...
    public static class IEnumerableExtensions
    {
        ...
        public static GridData<T> GetGridData<T>(this IEnumerable<T> collection, GridParams gridParams, string orderModifier = "") where T : class
        {
        ...
        public static List<T> GetRecords<T>(this IEnumerable<T> collection, GridParams gridParams, string orderModifier = "")
        {
            return collection.AsQueryable().OrderBy(gridParams.Ordering() + orderModifier).Skip((gridParams.Page - 1) * gridParams.Rows).Take(gridParams.Rows).ToList();
        }
    }
}
