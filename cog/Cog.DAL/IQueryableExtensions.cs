...
namespace Cog.DAL
{
    ...
    public static class IQueryableExtensions
    {
        ...
        public static IQueryable<T> Chain<T>(this IQueryable<T> queryable, ChainType type, params Expression<Func<T, bool>>[] conditions)
        {
        ...
        public static async Task<GridData<T>> GetGridDataAsync<T>(this IQueryable<T> query, GridParams gridParams, string orderModifier = "") where T : class
        {
        ...
        /// <param name="orderModifier">Modifier for search order.</param>
        /// <returns>GridData container.</returns>
        public static GridData<T> GetGridData<T>(this IQueryable<T> query, GridParams gridParams, string orderModifier = "") where T : class
        {
            query = BuildGridDataQuery(query, gridParams);
        ...
        public static async Task<List<T>> GetRecordsAsync<T>(this IQueryable<T> query, GridParams gridParams, string orderModifier = "") where T : class
        {
            return await query.OrderBy(gridParams.Ordering() + orderModifier).Skip((gridParams.Page - 1) * gridParams.Rows).Take(gridParams.Rows).ToListAsync();
        }
        ...
        /// <param name="orderModifier">Additional sort parameters.</param>
        /// <returns>List of records.</returns>
        public static List<T> GetRecords<T>(this IQueryable<T> query, GridParams gridParams, string orderModifier = "") where T : class
        {
            return query.OrderBy(gridParams.Ordering() + orderModifier).Skip((gridParams.Page - 1) * gridParams.Rows).Take(gridParams.Rows).ToList();
        }
        ...
        {
            if (!gridParams.IsValidPropertyName<T>())
            {
                throw new CogSecurityException("sql injection attempt");
            }
                ...
                foreach (var f in gridParams.Filters)
                {
                    ...
                    switch (f.Type)
                    {
                        ...
                        case GridParams.Filter.FilterTypes.Like:
                            foreach (var str in f.Values)
                            {
                                query = query.Where($"{f.Idx}.Contains(@0)", str);
                            }
                            break;
                    }
                }
