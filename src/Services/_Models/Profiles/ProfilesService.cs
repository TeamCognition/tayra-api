    ...
    { 
        public async Task<ProfilePulse> GetProfilePulseDTO(OrganizationDbContext db, Guid profileId, CancellationToken token)
        {
            ...
            var tasks = await (from t in db.Tasks
                         ...
                         where t.Status == WorkUnitStatuses.Started || (t.Status == WorkUnitStatuses.Done && t.LastModifiedDateId >= yesterdayDateId)
                         select new
                         {
            ...
            {
                InProgress = tasks.Select(x => x.DTO).Where(x => x.Status == WorkUnitStatuses.Started).ToArray(),
