...
{
    public static class ProfileMetricsGenerator
    {
        public static List<ProfileMetric> GenerateAndSave(OrganizationDbContext organizationDb, DateTime fromDay, LogService logService)
        {
            ...
            if (existing > 0)
            {
                logService.Log(nameof(ProfileMetricsGenerator), $"deleting {existing} records from database");
                organizationDb.Database.ExecuteSqlInterpolated($"delete from ProfileReportsDaily where {nameof(ProfileMetric.DateId)} = {dateId}");
            ...
            logService.Log(nameof(ProfileMetricsGenerator), $"{metricsToInsert.Count} new profile metrics saved to database.");
