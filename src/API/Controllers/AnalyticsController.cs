...
namespace Tayra.API.Controllers
{
    public class AnalyticsController : BaseController
    {
        ...
        [HttpGet("metrics")]
        public Dictionary<int, MetricValue> GetMetrics([FromQuery] int[] m, [FromQuery] Guid entityId, [FromQuery] EntityTypes entityType, [FromQuery] string period)
        {
        ...
        public Dictionary<int, AnalyticsMetricWithBreakdownDto> GetAnalyticsWithBreakdown([FromQuery] int[] m, [FromQuery] Guid entityId, [FromQuery] EntityTypes entityType, [FromQuery] string period)
        {
            var datePeriod = new DatePeriod(period);
            var metricTypes = m.Select(x => MetricType.FromValue(x)).ToArray();
        ...
        [HttpGet("rawMetrics")]
        public TableData GetRawMetrics([FromQuery] int m, [FromQuery] Guid entityId, [FromQuery] EntityTypes entityType, [FromQuery] string period)
        {
            var datePeriod = new DatePeriod(period);
        ...
        [HttpGet("workFocus")]
        public ActionResult<AnalyticsWorkFocusDto> GetWorkFocus([FromQuery] string period, [FromQuery] Guid segmentId,
    [FromQuery] Guid profileId)
        {
            ...
            var teamHeatMetric = AnalyticsService.GetMetricsWithIterationSplit(
                ...
                {
            MetricType.Heat
                }, segmentId, EntityTypes.Segment, datePeriod);
            ...
            var tasks = OrganizationContext.Tasks.Where(x => x.Status == TaskStatuses.Done && x.TeamId == segmentId && x.LastModifiedDateId >= datePeriod.FromId && x.LastModifiedDateId <= datePeriod.ToId)
            .Select(x => new
            {
            ...
            var teamDoneTasks = tasks.Count(x => x.TeamId == segmentId);

            var userNewWorkBug = tasks.Count(x => x.Type != TaskTypes.Bug && x.AssigneeProfileId == profileId);
            var teamNewWorkBug = tasks.Count(x => x.Type != TaskTypes.Bug && x.TeamId == segmentId);
            ...
            var teamComplexityIndex = tasks.Count(x => x.Complexity <= 2 && x.TeamId == segmentId) - tasks.Count(x => x.Complexity >= 3 && x.TeamId == segmentId);
