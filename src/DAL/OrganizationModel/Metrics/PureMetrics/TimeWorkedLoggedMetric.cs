...
{
    public class TimeWorkedLoggedMetric : PureMetric
    {
        public TimeWorkedLoggedMetric(string name, int value) : base(name, value)
        {

        }
        public MetricShardWEntity Create(IEnumerable<WorkUnit> tasks, int dateId, Guid segmentId) => new MetricShardWEntity(tasks.Sum(x => x.TimeSpentInMinutes) ?? 0f, dateId, segmentId, this);
