...
namespace Tayra.Metrics
{
    public class TimeWorkedMetric : PureMetric
    {
        ...
        public MetricShardWEntity Create(IEnumerable<WorkUnit> tasks, int dateId, Guid segmentId) => new MetricShardWEntity(tasks.Sum(x => x.TimeSpentInMinutes ?? x.AutoTimeSpentInMinutes) ?? 0f, dateId, segmentId, this);
