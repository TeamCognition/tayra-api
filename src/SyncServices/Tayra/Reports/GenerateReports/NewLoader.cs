        ...
        public static List<ProfileReportDaily> GenerateProfileReportsDaily(OrganizationDbContext organizationDb, DateTime fromDay, LogService logService, params int[] segmentIds)
        {
            ...
            foreach (var segmentId in segmentIds)
            {
                ...
                var taskStats = (from t in organizationDb.Tasks
                                 ...
                                 group t by t.AssigneeProfileId into g
                                 select new
                                 {
                                     ProfileId = g.Key,

                                     EffortScore = new EffortMetric(g.Sum(x => x.EffortScore) ?? 0f),

                                     Complexity = new ComplexityMetric(g.Sum(x => x.Complexity)),

                                     Completed = new WorkUnitsCompleted(g.Count()),
                                     CompletedNames = g.Select(x => x.Summary).ToArray(),

                                     ProductionBugsFixed = g.Count(x => x.IsProductionBugFixing),

                                     ProductionBugsCaused = g.Count(x => x.IsProductionBugCausing),

                                     BugSeverity = g.Where(x => x.IsProductionBugCausing).Select(x => x.BugSeverity),
                                     BugPopulationAffect = g.Where(x => x.IsProductionBugCausing).Select(x => x.BugPopulationAffect),

                                     Saves = g.Count(x => x.IsProductionBugFixing && x.BugSeverity > 3),

                                     Turnovers = 0,

                                     Tackles = 0,

                                     MinutesSpent = g.Sum(x => x.TimeSpentInMinutes ?? x.AutoTimeSpentInMinutes), //TODO: those should could not completed tasks as well
                                 }).ToList();
