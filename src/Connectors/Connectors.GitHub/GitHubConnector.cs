    ...
    public class GitHubConnector : BaseOAuthConnector
    {
        ...
        public override Integration Authenticate(int profileId, ProfileRoles profileRole, int segmentId, string userState)
        {
            if (HttpContext?.Request != null)
            {
                ...
                var userTokenData = GitHubService.GetUserAccessToken(code, GetCallbackUrl(userState))?.Data;
                ...
                var reqq = GitHubService.GetInstallationAccessToken(installationId, GHAPP_ID, GHAPP_RSA_KEY);

                var installationToken = reqq?.Data.AccessToken;
                ...
                if (profileRole != ProfileRoles.Member && userTokenData != null)
                {
